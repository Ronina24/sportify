...
function showTab(pageNum) {
  ...
  x[currentTab].classList.add("tab-active");
...
function fixStepIndicator(n) {
  var i,
    stepper = document.getElementsByClassName("stepper-item");
  for (i = 0; i < stepper.length; i++) {
    stepper[i].className = stepper[i].className.replace(" active", "");
  }
  stepper[n].className += " completed";
  stepper[n].className += " active";
}
